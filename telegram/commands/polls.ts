...
polls.on('poll_answer', async (ctx) => {
    ...
    const encuesta = await prisma.poll.findUnique({
        where: {
            id,
        },
        include: {
    ...
    if (encuesta !== null && ctx.pollAnswer.user) {
        ...
        await ctx.api.sendMessage(encuesta.chat, text, { parse_mode: 'HTML' }).catch(console.log)
    }
})
